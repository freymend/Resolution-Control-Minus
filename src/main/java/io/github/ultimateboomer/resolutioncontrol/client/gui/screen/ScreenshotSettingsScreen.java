...
public class ScreenshotSettingsScreen extends SettingsScreen {
  private static final double[] scaleValues = {0.1, 0.25, 0.5, 1.0, 2.0, 3.0, 4.0, 6.0, 8.0, 16.0};
  ...
  protected void init() {
    ...
    toggleOverrideSizeButton =
        new ButtonWidget.Builder(
            ...
            .dimensions(centerX + 20, centerY - 40, 50, 20)
            .build();
    this.addDrawableChild(toggleOverrideSizeButton);

    toggleAlwaysAllocatedButton =
        new ButtonWidget.Builder(
                getStateText(mod.isScreenshotFramebufferAlwaysAllocated()),
                button -> {
                  mod.setScreenshotFramebufferAlwaysAllocated(
                      !mod.isScreenshotFramebufferAlwaysAllocated());
                  button.setMessage(getStateText(mod.isScreenshotFramebufferAlwaysAllocated()));
                })
            .dimensions(centerX + 20, centerY - 20, 50, 20)
            .build();
    ...
    increaseButton =
        new ButtonWidget.Builder(increaseText, button -> multiply(2.0))
            .dimensions(centerX - 10 - 60, centerY + 35, 20, 20)
            .build();
    this.addDrawableChild(increaseButton);

    decreaseButton =
        new ButtonWidget.Builder(decreaseText, button -> multiply(0.5))
            .dimensions(centerX + 10 - 60, centerY + 35, 20, 20)
            .build();
    this.addDrawableChild(decreaseButton);

    resetButton =
        new ButtonWidget.Builder(resetText, button -> resetSize())
            .dimensions(centerX + 30 - 60, centerY + 35, 20, 20)
            .build();
  ...
  public void render(DrawContext context, int mouseX, int mouseY, float delta) {
    ...
    drawLeftAlignedString(context, "\u00a78x", centerX - 42, centerY + 12, 0x000000);

    drawLeftAlignedString(
        context,
        "\u00a78"
            + text("settings.main.estimate").getString()
            + " "
            + RCUtil.formatMetric(estimatedSize)
            + "B",
        centerX + 25,
        centerY + 12,
        0x000000);
  ...
  private void multiply(double mul) {
    if (NumberUtils.isParsable(widthTextField.getText())
      ...
      widthTextField.setText(
          String.valueOf((int) Math.abs(Double.parseDouble(widthTextField.getText()) * mul)));
      heightTextField.setText(
          String.valueOf((int) Math.abs(Double.parseDouble(heightTextField.getText()) * mul)));
      calculateSize();
    }
  }
